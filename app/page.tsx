...
// Import the KnowledgeBaseModal component and utility functions
import KnowledgeBaseModal from './components/KnowledgeBaseModal';
import { getCsrfToken } from './components/utils';

// Preload commonly used icons to prevent flickering
(() => {
  const preloadImages = ['report-thin-icon.png', 'table-thin-icon.png'];
  preloadImages.forEach(imgName => {
    const img = new Image();
...
export default function Page() {
	const availableModels = JSON.parse(document.getElementById('available-models').textContent);

	const [messages, setMessages] = useState(() => JSON.parse(document.getElementById('initial-messages').textContent));
	const prevMessages = useRef([]);
	const [inputMessage, setInputMessage] = useState('');
	const [isStreaming, setIsStreaming] = useState(false);
	const isStreamingRef = useRef(isStreaming);
	const chatContainerRef = useRef(null);
	const [threadId, setThreadId] = useState(() => JSON.parse(document.getElementById('thread-id').textContent));
	const [model, setModel] = useState(() => {
		const modelOverride = JSON.parse(document.getElementById('model-override').textContent);
		if (modelOverride && availableModels[modelOverride]) {
			return modelOverride;
		}
		// Check thread data or localStorage
		const threadDataElement = document.getElementById('thread-data');
		const thread = threadDataElement ? JSON.parse(threadDataElement.textContent) : { model: null };
	...
	const [connectedApps] = useState(() => JSON.parse(document.getElementById('connected-apps').textContent));
	...
	useEffect(() => {
		const selectedText = document.getElementById('selected-data-source')?.textContent;
		const selected = selectedText ? parseInt(JSON.parse(selectedText)) : null;
		const threadDataElement = document.getElementById('thread-data');
		const thread = threadDataElement ? JSON.parse(threadDataElement.textContent) : { connection_ids: [] };
